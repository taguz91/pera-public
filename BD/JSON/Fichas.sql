...
---Consultamos toda la ficha de una persona ficha con preguntas libres etc

---Consultamos con respuestas libres
SELECT array_to_json(
	array_agg(s.*)
) AS secciones FROM (
	SELECT
...
SELECT array_to_json(
	...
	sf.id_tipo_ficha = (
		...
		FROM public."PermisoIngresoFichas" pif,
		public."PersonaFicha" prf
		WHERE id_persona_ficha = 11 AND
		pif.id_permiso_ingreso_ficha = prf.id_permiso_ingreso_ficha
	)
) AS s;


--Consultamos todos los tipos de fichas
